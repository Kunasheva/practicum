...
def print_help_string():
    help_string = r'''|wf             | --> Name of the working folder
...
|crtfl [file_name] ..                 | --> Create files
|wrtfl [file_name]                    | --> Write to file, stream string input
|rdfl [file_name] ..                  | --> Displays files
|rmfl [file_name] ..                  | --> Remove files
|cpflfd [file_from] [file_to]         | --> Copy file_name to folder/new_file
|rpls [file_folder] [file_folder]     | --> replace file(folder) 
|rnm [file_folder]                    | --> Rename file(folder)
|exit                                 | --> Exit
|help                                 | --> Command list'''
    print(help_string)
...
def command_prompt():
    ...
    while True:
        command = input('Command:/'+'/'.join(current_directory[1:])+'$ ').split()
        if command[0] == 'exit':
            break
        try:
